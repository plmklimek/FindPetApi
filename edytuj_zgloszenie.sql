CREATE DEFINER=`root`@`localhost` PROCEDURE `edytuj_zgloszenie`(_id INT,_idUzytkownik INT ,_idzwierzecia INT,_tresc VARCHAR(90) CHARSET utf8mb4,_komentarz INT,_typ_zgloszenia VARCHAR(1) CHARSET utf8mb4,_typ_zwierzecia VARCHAR(4) CHARSET utf8mb4,_rasa VARCHAR(25) CHARSET utf8mb4,_wielkosc VARCHAR(6) CHARSET utf8mb4,_kolor_siersci VARCHAR(25) CHARSET utf8mb4,_znaki_szczegolne VARCHAR(45) CHARSET utf8mb4,_data_stworzenia DATETIME,_data_zaginiecia DATETIME,_latitude FLOAT,_longtitude FLOAT, _obszar  INT, _zdjecie1 VARCHAR(200), _zdjecie2 VARCHAR(200),_zdjecie3 VARCHAR(200),_zdjecie4 VARCHAR(200))
BEGIN
...
DECLARE idzdj_3 INT;
DECLARE idzdj_4 INT;
SELECT A.idZwierzecia INTO idzwierz FROM zwierzeta A JOIN posty B ON A.idZwierzecia = B.Zwierzeta_idZwierzecia WHERE B.idPosty = _id;
...
START TRANSACTION;
	...
	UPDATE zwierzeta SET typ_zgloszenia = _typ_zgloszenia, typ_zwierzecia = _typ_zwierzecia , rasa = _rasa, wielkosc = _wielkosc, kolor_siersci = _kolor_siersci , znaki_szczegolne = _znaki_szczegolne , data_zaginiecia = _data_zaginiecia WHERE idZwierzecia = idzwierz;
    UPDATE posty SET tresc = _tresc,id_komentarz = _komentarz,Zwierzeta_idZwierzecia =  _idzwierzecia,Uzytkownicy_idUzytkownik = _idUzytkownik,data_stworzenia = _data_stworzenia WHERE idPosty = _id;
